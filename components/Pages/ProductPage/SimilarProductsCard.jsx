...
const SimilarProductsCard = (props) => {
  ...
  return (
    <div className={st.similarRobotsCard}>
      ...
      <div className={st.wishlistBox}>
        {wishlist ? (
          ...
          >
            ...
            <RemoveFromWishlist fill="#313130" />
          </main>
        )}
      </div>
      ...
      {props.singlePrice !== "" ? (
        <h2 className={st.price}> {props.singlePrice}</h2>
      ) : null}
      {props.variablePrice !== "" ? (
        <h2 className={st.price}> {props.variablePrice}</h2>
      ) : null}
      {props.beforeSalePrice !== "" ? (
        <h2 className={st.price}>
          {" "}
          <del>{props.beforeSalePrice}</del> {props.afterSalePrice}{" "}
        </h2>
      ) : null}
      {props.varient ? (
        <div className={st.varientBox}>
          <label htmlFor="">Variante</label>
          <select className="ds-input" name="" id="">
            <option value="Universal Robots Kit">Universal Robots Kit</option>
            <option value="Universal Robots Kit">Universal Robots Kit</option>
          </select>
        </div>
      ) : null}
